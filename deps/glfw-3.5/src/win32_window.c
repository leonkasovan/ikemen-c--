...
//
static void win32_acquireMonitor(_GLFWwindow* window) {
...
//
static void win32_releaseMonitor(_GLFWwindow* window) {
    ...
    if (!_glfw.win32.acquiredMonitorCount) {
        ...
        // HACK: Restore mouse trail length saved in win32_acquireMonitor
...
static LRESULT CALLBACK windowProc(HWND hWnd, UINT uMsg, WPARAM wParam, LPARAM lParam) {
    ...
    case WM_SIZE:
    {
        ...
        if (window->monitor && window->win32.iconified != iconified) {
            if (iconified)
                win32_releaseMonitor(window);
            else {
                win32_acquireMonitor(window);
                fitToMonitor(window);
            }
        }
