...
// ==========================================
// OpenGL Version Detection & Macros
// ==========================================

#define OPENGL_VERSION_MAJOR 3
#define OPENGL_VERSION_MINOR 3

// Utility macro for OpenGL call error checking (debug builds only)
#ifdef DEBUG
    #define GL_CHECK_ERROR() \
        do { \
            GLenum err = glGetError(); \
            if (err != GL_NO_ERROR) { \
                std::cerr << "OpenGL Error: " << std::hex << err << " at " \
...
// ==========================================
// OpenGL-Specific Shader Program
// ==========================================

class ShaderProgram_GL : public IShaderProgram {
public:
...
public:
    // Constructor/Destructor
    Texture_GL(int32_t width, int32_t height, int32_t depth, bool filter, uint32_t handle);
    ~Texture_GL();
    ...
    void CopyData(const Texture_GL* src);
...
public:
    ...
    // ===== Texture Operations =====
    std::shared_ptr<ITexture> newTexture(int32_t width, int32_t height, int32_t depth, bool filter) override;
    std::shared_ptr<ITexture> newPaletteTexture() override;
    std::shared_ptr<ITexture> newPaletteTextureArray(int32_t layers) override;
    std::shared_ptr<ITexture> newModelTexture(int32_t width, int32_t height, int32_t depth, bool filter) override;
    std::shared_ptr<ITexture> newDataTexture(int32_t width, int32_t height) override;
    std::shared_ptr<ITexture> newHDRTexture(int32_t width, int32_t height) override;
    std::shared_ptr<ITexture> newCubeMapTexture(int32_t widthHeight, bool mipmap, int32_t lowestMipLevel) override;
    ...
    // ===== OpenGL-Specific Public Methods =====
    ...
    void ConfigureForOpenGLVersion();
...
private:
    ...
    // Shader programs
    std::shared_ptr<ShaderProgram_GL> spriteShader;
    std::shared_ptr<ShaderProgram_GL> modelShader;
    std::shared_ptr<ShaderProgram_GL> shadowMapShader;
    std::shared_ptr<ShaderProgram_GL> panoramaToCubeMapShader;
    std::shared_ptr<ShaderProgram_GL> cubemapFilteringShader;
    std::vector<std::shared_ptr<ShaderProgram_GL>> postShaderSelect;
    ...
    std::shared_ptr<ShaderProgram_GL> newShaderProgram(const std::string& vert, const std::string& frag,
    ...
    void SetupVertexAttributes(const std::shared_ptr<ShaderProgram_GL>& shader, 
...
inline void Texture_GL::Bind(GLenum target) const {
    glBindTexture(target, handle);
}

inline void Texture_GL::Unbind(GLenum target) const {
    glBindTexture(target, 0);
}

inline void Texture_GL::SetFilterMode(bool linearFilter) {
    glBindTexture(textureTarget, handle);
    GLint minFilter = linearFilter ? GL_LINEAR : GL_NEAREST;
...
inline uint32_t Renderer_GL::GetSpriteShaderProgram() const {
    return spriteShader ? spriteShader->GetProgram() : 0;
}

inline uint32_t Renderer_GL::GetModelShaderProgram() const {
    return modelShader ? modelShader->GetProgram() : 0;
}

inline uint32_t Renderer_GL::GetShadowMapShaderProgram() const {
    return shadowMapShader ? shadowMapShader->GetProgram() : 0;
}

inline void Renderer_GL::PrintInfo() {
    PrintGLInfo();
}

inline void Renderer_GL::PrintCapabilities() {
    PrintGLCapabilities();
}
